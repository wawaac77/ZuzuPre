...
- (void)viewDidLoad {
  ...
  self.detailArr =@[@{@"section0" :@[ZBLocalized(@"Clear chat history", nil)]}];
  [self setupNavigation];
  [self setupTableView];
}

- (void)setupNavigation {
    self.title= ZBLocalized(@"Chat Detail", nil);
...
#pragma mark -- 加入好友到群
- (void)addFriend {
  UIAlertView *alerView = [[UIAlertView alloc]initWithTitle: ZBLocalized(@"Add friend into group", nil)
...
- (void)tapToClearChatRecord {
  UIAlertView *alerView = [[UIAlertView alloc] initWithTitle:ZBLocalized(@"Clear chat history", nil)
                                                     message:@""
                                                    delegate:self
                                           cancelButtonTitle:ZBLocalized(@"Cancel", nil)
                                           otherButtonTitles:ZBLocalized(@"Confirm", nil), nil];
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
  ...
  switch (alertView.tag) {
    ...
    {
      [MBProgressHUD showMessage:ZBLocalized(@"Add friend into group", nil) toView:self.view];
      ...
      [JMSGGroup createGroupWithName:@"" desc:@"" memberArray:@[((JMSGUser *)self.conversation.target).username,[alertView textFieldAtIndex:0].text] completionHandler:^(id resultObject, NSError *error) {
        ...
        if (error == nil) {
          [JMSGConversation createGroupConversationWithGroupId:tmpgroup.gid completionHandler:^(id resultObject, NSError *error) {
            if (error == nil) {
              [MBProgressHUD showMessage:ZBLocalized(@"Success", nil) view:self.view];
