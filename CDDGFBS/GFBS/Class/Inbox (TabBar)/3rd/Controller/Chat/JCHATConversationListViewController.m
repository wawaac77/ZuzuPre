...
- (void)setupBubbleView {
    _addBgView = [[UIImageView alloc] initWithFrame:CGRectMake(kApplicationWidth - 120, 1, 120, 80)];
    [_addBgView setBackgroundColor:[UIColor clearColor]];
    [_addBgView setUserInteractionEnabled:YES];
    UIImage *frameImg = [UIImage imageNamed:@"frame_darkgrey"];
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if (!appDelegate.isDBMigrating) {
        ...
        NSLog(@"is DBMigrating don't get allconversations");
        [MBProgressHUD showMessage:ZBLocalized(@"Updating database", nil) toView:self.view];
    }
...
- (void)netWorkConnectClose {
    ...
    _titleLabel.text = ZBLocalized(@"Not Connected", nil);
    _titleActivity.hidden = YES;
}
...
- (void)netWorkConnectSetup {
    ...
    _titleLabel.text = ZBLocalized(@"Receiving...", nil);
...
- (void)isConnecting {
    ...
    _titleLabel.text = ZBLocalized(@"Connecting...", nil);
    _titleActivity.hidden = NO;
}
...
- (void)addBtn {
    ...
    [btn setTitle: ZBLocalized(@"New Chat", nil) forState:UIControlStateNormal];
    [btn addTarget:self action:@selector(btnClick:) forControlEvents:UIControlEventTouchUpInside];
    btn.tag = 1 + 100;
    [btn setFrame:CGRectMake(10, i*30+30, 100, 30)];
    ...
    for (NSInteger i=0; i<2; i++) {
        UIButton *btn =[UIButton buttonWithType:UIButtonTypeCustom];
        if (i==0) {
            [btn setTitle:ZBLocalized(@"Group Chat", nil) forState:UIControlStateNormal];
        }
        if (i==1) {
            [btn setTitle: ZBLocalized(@"New Chat", nil) forState:UIControlStateNormal];
        }
...
- (void)btnClick :(UIButton *)btn {
    ...
    if (btn.tag == 100) {
        ...
        UIAlertView *alerView =[[UIAlertView alloc] initWithTitle:ZBLocalized(@"Add Contact", nil)
                                                          message:ZBLocalized(@"Please input username or email", nil)
                                                         delegate:self
                                                cancelButtonTitle:ZBLocalized(@"Cancel", nil)
                                                otherButtonTitles:ZBLocalized(@"Confirm", nil), nil];
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    ...
    } else if (buttonIndex == 1)
    {
        if ([[alertView textFieldAtIndex:0].text isEqualToString:@""]) {
            [MBProgressHUD showMessage:ZBLocalized(@"Please input username or email", nil) view:self.view];
            return;
        }
        ...
        [JMSGConversation createSingleConversationWithUsername:[alertView textFieldAtIndex:0].text appKey:JMESSAGE_APPKEY completionHandler:^(id resultObject, NSError *error) {
            ...
            if (error == nil) {
                ...
                [MBProgressHUD showMessage:ZBLocalized(@"The user does not exist", nil) view:self.view];
            }
        }];
    }
}
