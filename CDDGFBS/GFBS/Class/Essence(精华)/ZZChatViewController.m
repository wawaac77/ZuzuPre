...
@property(assign, nonatomic) FIRDatabaseHandle *newMessageRefHandle;
...
- (void)viewDidLoad {
     ...
     */
    self.demoData = [[ZZMessageModel alloc] init];
                      ...
                      date:(NSDate *)date
{
    ...
    //****** push to firebase ******//
    //[_messageRef.database.reference.URL setValue:@"https://<my-firebase-app>.firebaseio.com/messages/" forKey:@"messages"];
    //[_messageRef.ref.URL setValue: @"https://<my-firebase-app>.firebaseio.com/messages/" forKey:@"messages"];
    
    FIRDatabaseReference *itemRef = _messageRef.childByAutoId;
    NSLog(@"senderId %@", senderId);
    NSLog(@"senderName %@", senderDisplayName);
    NSLog(@"text %@", text);
    NSDictionary *messageItem = @{@"senderId" : senderId, @"senderName" : senderDisplayName, @"text" : text};
...
- (FIRDatabaseReference *)messageRef {
    if (_messageRef == nil) {
        _messageRef = [self.channelRef child:@"messages"];
    }
